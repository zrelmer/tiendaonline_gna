<section>
    <div class="container-fluid-lg">
        <div class="title">
            <h2>Mejores Ofertas</h2>
        </div>

        <div class="row">
            <div class="col-12">
                <div class="three-slider arrow-slider ratio_65">

                    <!-- Oferta 1 -->
                    <div>
                        <div class="offer-banner hover-effect">
                            <img src="{{ asset('assets/images/veg-3/value/1.png') }}"
                                 class="img-fluid bg-img blur-up lazyload" alt="Ofertas tecnología">

                            <div class="banner-detail">
                                <h5 class="theme-color">Compra más y ahorra</h5>
                                <h6>Accesorios tecnológicos</h6>
                            </div>

                            <div class="offer-box">
                                <a href="{{ route('shop.index') }}"
                                   class="btn-category btn theme-bg-color text-white">
                                    Comprar Ahora
                                </a>
                            </div>
                        </div>
                    </div>

                    <!-- Oferta 2 -->
                    <div>
                        <div class="offer-banner hover-effect">
                            <img src="{{ asset('assets/images/veg-3/value/2.png') }}"
                                 class="img-fluid bg-img blur-up lazyload" alt="Descuentos">

                            <div class="banner-detail">
                                <h5 class="theme-color">¡Ahorra más!</h5>
                                <h6>Celulares en oferta</h6>
                            </div>

                            <div class="offer-box">
                                <a href="{{ route('shop.index') }}"
                                   class="btn-category btn theme-bg-color text-white">
                                    Comprar Ahora
                                </a>
                            </div>
                        </div>
                    </div>

                    <!-- Oferta 3 -->
                    <div>
                        <div class="offer-banner hover-effect">
                            <img src="{{ asset('assets/images/veg-3/value/3.png') }}"
                                 class="img-fluid bg-img blur-up lazyload" alt="Promociones">

                            <div class="banner-detail">
                                <h5 class="theme-color">¡Ofertas calientes!</h5>
                                <h6>Laptops y tecnología</h6>
                            </div>

                            <div class="offer-box">
                                <a href="{{ route('shop.index') }}"
                                   class="btn-category btn theme-bg-color text-white">
                                    Comprar Ahora
                                </a>
                            </div>
                        </div>
                    </div>

                    <!-- Oferta 4 -->
                    <div>
                        <div class="offer-banner hover-effect">
                            <img src="{{ asset('assets/images/veg-3/value/1.png') }}"
                                 class="img-fluid bg-img blur-up lazyload" alt="Descuentos especiales">

                            <div class="banner-detail">
                                <h5 class="theme-color">Grandes descuentos</h5>
                                <h6>Productos destacados</h6>
                            </div>

                            <div class="offer-box">
                                <a href="{{ route('shop.index') }}"
                                   class="btn-category btn theme-bg-color text-white">
                                    Comprar Ahora
                                </a>
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </div>
    </div>
</section>
